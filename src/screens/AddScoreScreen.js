...
const AddScoreScreen = ({navigation, route}) => {
  const { update } = useContext(ItemContext);
  //NOTE, adding a score, means updating the score card, because
  //each scorecard has an empty object. The object looks like:
  ...
  const [newScore, setNewScore] = useState(item.score)
  ...
  function addScore () {
    ...
    {
      let object = {End: newScore.length+1, Team1Score: parseInt(newTeam1Score), Team2Score: 0};
      newScore.push(object)
    }
    else if (newTeam2Score > newTeam1Score)
    {
      let object = {End: newScore.length+1, Team1Score: 0, Team2Score: parseInt(newTeam2Score)};
      newScore.push(object)    
    }
  }
  ...
  <View>
          ...
          <Button title="Submit Item" onPress={() => {
            ...
            update(item.id, item.compN, item.rinkNo, item.T1, item.T2, item.T1P1, item.T1P2, item.T1P3, item.T1P4, item.T1Skip, item.T2P1, item.T2P2, item.T2P3, item.T2P4, item.T2Skip,
              item.date, newScore, () => navigation.pop());
          }} />
